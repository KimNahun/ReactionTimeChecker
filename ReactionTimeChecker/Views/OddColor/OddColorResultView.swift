// Views/OddColor/OddColorResultView.swift
import SwiftUI
import UIKit
import TopDesignSystem

struct OddColorResultView: View {
    let session: OddColorSession
    let onPlayAgain: () -> Void
    var onHome: (() -> Void)? = nil
    @State private var viewModel: OddColorResultViewModel
    @Environment(\.designPalette) var palette

    init(session: OddColorSession, onPlayAgain: @escaping () -> Void, onHome: (() -> Void)? = nil) {
        self.session = session; self.onPlayAgain = onPlayAgain; self.onHome = onHome
        self._viewModel = State(initialValue: OddColorResultViewModel(session: session))
    }
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: DesignSpacing.lg) {
                    Text(String(localized: "'Odd Color' Results")).font(.ssTitle1).foregroundStyle(palette.textPrimary).padding(.top, DesignSpacing.xl)
    ...
    private var shareButtons: some View {
        VStack(spacing: DesignSpacing.sm) {
            VStack(spacing: DesignSpacing.sm) {
                ...
                HStack(spacing: 24) {
                    Button { KakaoShareService.shareOddColor(session: session, grade: viewModel.grade, percentile: viewModel.percentile) } label: {
    ...
    private func shareText() {
        ...
        let text = isKorean
            ? "\(viewModel.grade.emoji) \(name)님의 색상 찾기: \(session.roundsCompleted)라운드 · 상위 \(viewModel.percentile)% — QuickTap"
            : "\(viewModel.grade.emoji) \(name)'s Odd Color: Round \(session.roundsCompleted) · Top \(viewModel.percentile)% — QuickTap"
