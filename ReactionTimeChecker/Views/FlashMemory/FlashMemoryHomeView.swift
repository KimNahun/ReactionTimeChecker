// Views/FlashMemory/FlashMemoryHomeView.swift
import SwiftUI
import TopDesignSystem

struct FlashMemoryHomeView: View {
    let onStart: () -> Void
    var onBack: (() -> Void)? = nil
            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: DesignSpacing.lg) {
                        ...
                        Text(String(localized: "Flash Memory")).font(.ssTitle1).foregroundStyle(palette.textPrimary)
                        Text("🔦").font(.system(size: 72))
                        Text(String(localized: "Remember the flashing numbers!")).font(.ssBody).foregroundStyle(palette.textSecondary)
                        SurfaceCard(elevation: .raised) {
                            VStack(spacing: DesignSpacing.md) {
                                ...
                                VStack(alignment: .leading, spacing: DesignSpacing.sm) {
                                    ruleRow(icon: "1.circle.fill", text: String(localized: "3 numbers flash briefly on screen"))
                                    ruleRow(icon: "2.circle.fill", text: String(localized: "Pick the 3 correct numbers from 5 choices"))
                                    ruleRow(icon: "3.circle.fill", text: String(localized: "Wrong = game over! Display time gets shorter"))
                                }
