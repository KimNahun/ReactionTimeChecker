// Views/FlashMemory/FlashMemoryResultView.swift
import SwiftUI
import UIKit
import TopDesignSystem

struct FlashMemoryResultView: View {
    let session: FlashMemorySession
    let onPlayAgain: () -> Void
    var onHome: (() -> Void)? = nil
    @State private var viewModel: FlashMemoryResultViewModel
    @Environment(\.designPalette) var palette

    init(session: FlashMemorySession, onPlayAgain: @escaping () -> Void, onHome: (() -> Void)? = nil) {
        self.session = session; self.onPlayAgain = onPlayAgain; self.onHome = onHome
        self._viewModel = State(initialValue: FlashMemoryResultViewModel(session: session))
    }
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: DesignSpacing.lg) {
                    Text(String(localized: "'Flash Memory' Results")).font(.ssTitle1).foregroundStyle(palette.textPrimary).padding(.top, DesignSpacing.xl)
                    ...
                    if viewModel.stage >= .score {
                        SurfaceCard(elevation: .raised) {
                            VStack(spacing: DesignSpacing.sm) {
                                ...
                                Text(String(format: String(localized: "Last display: %.1fs"), session.lastDisplayDuration))
                    ...
                    if viewModel.stage >= .shareButton {
                        shareButtons.transition(.opacity)
                    }
    ...
    private var shareButtons: some View {
        VStack(spacing: DesignSpacing.sm) {
            VStack(spacing: DesignSpacing.sm) {
                ...
                HStack(spacing: 24) {
                    Button { KakaoShareService.shareFlashMemory(session: session, grade: viewModel.grade, percentile: viewModel.percentile) } label: {
    ...
    private func shareText() {
        ...
        let text = isKorean
            ? "\(viewModel.grade.emoji) \(name)님의 순간 포착: \(session.roundsCompleted)라운드 · 상위 \(viewModel.percentile)% — QuickTap"
            : "\(viewModel.grade.emoji) \(name)'s Flash Memory: Round \(session.roundsCompleted) · Top \(viewModel.percentile)% — QuickTap"
